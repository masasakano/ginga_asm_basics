...
contains
  ...
  !------------------------------------------------------------
  function get_asmevt_summed_chan(funit, chan_l_h, nrows) result(retchans)
    implicit none
...
!if (IS_DEBUG())                   print *,'DEBUG:3270: 110=',retchans(110, :)
  end function get_asmevt_summed_chan

  !------------------------------------------------------------
  ! Read ASM fits and returns required Arrays to output.
  !------------------------------------------------------------
  subroutine asmevt_time_row_det_band(fname, chans, artime, outchans)
    implicit none
...
!if (IS_DEBUG()) print *,'DEBUG:3245: out-sizes=',size(outchans, 1),' s2=',size(outchans, 2),' s3=',size(outchans, 3)
    ...
    do iband=1, size(chans, 2)
      outchans(:,:,iband) = get_asmevt_summed_chan(funit, chans(:, iband), nrows)
!if (IS_DEBUG()) print *,'DEBUG:3369: iband=',iband,' sum=',sum(outchans(:,2,iband))
  ...
  end subroutine asmevt_time_row_det_band

  !------------------------------------------------------------
  ! Write QDP/PCO
  !------------------------------------------------------------
  subroutine write_qdp_pco(fname, outroot, chans, artime, outchans, status)
    implicit none
...
!if (IS_DEBUG()) print *,'DEBUG:3157: out-after sum=',sum(outchans(:,2,1))
  ...
  end subroutine write_qdp_pco

  !------------------------------------------------------------
  ! Read ASM fits and write QDP/PCO (Parent subroutine)
  !------------------------------------------------------------
  subroutine read_asmevt_write_qdp(fname, outroot, chans) !, status)
    implicit none
    character(len=*), intent(in) :: fname, outroot  ! fname for ASM.fits
    integer, dimension(:,:), intent(in) :: chans  ! ((Low,High), i-th-band)
    integer :: status
    ...
    call asmevt_time_row_det_band(fname, chans, artime, outchans)
    call write_qdp_pco(fname, outroot, chans, artime, outchans, status)

    if (allocated(artime)) deallocate(artime)
    if (allocated(outchans)) deallocate(outchans)
  end subroutine read_asmevt_write_qdp
